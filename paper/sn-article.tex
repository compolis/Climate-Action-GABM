...
% NOTE: reach = validated backbone (Tier 1-3, audited in NB 45). Only the graded
%       reach-ladder sweep is still pending; numbers verified in paper/tables/.

The first form of advantage is reach: speaking to more people. Reach is the most
thoroughly tested lever in this study, the very manipulation on which the evaluation
above is built, so we treat it as the validated backbone of the analysis; only the
finer-grained reach ladder (a graded sweep from a quarter of the audience up to full
reach) is still in progress, and it will refine rather than overturn the numbers here.
Widening the reach gap in favour of one side produces a same-signed shift of the
majority toward that side. When the pro-climate minority reaches more citizens than the
sceptic minority, the same citizen ends up about $+0.42$ package-index points more
pro-climate (95\% CI $[+0.34, +0.51]$), same sign in all three seeds. This
whole-population figure is an average diluted by the citizens who hear only one side or
none: among the majority who hear both sides the shift is larger, about $+0.50$, while
among citizens who receive no broadcast at all it is still $+0.25$, a genuine spillover
carried through the peer network rather than by direct exposure. A silent no-broadcast
condition moves negligibly, and the shift is broad across all six individual policies
rather than driven by any single one (SI). Reaching more citizens moves the majority.
...
advantage ($+0.42$). That reach and frequency are separable and produce different
...
is the strongest and most thoroughly validated lever ($\approx +0.42$; the graded reach
ladder is still in progress), frequency a real but weaker one ($+0.16$
to $+0.28$), and targeting registers no measurable effect ($\approx 0$) within the
...
\midrule
Reach & reach more citizens & $+0.42\ [+0.34, +0.51]$ & $+0.50$ & same sign, 3/3 seeds & 12 \\
